...
if ($subGroup == "all") {
    ...
    $sql = "SELECT * FROM `itemscard` ORDER BY `stock` DESC";
...
?>
        <div onclick="addItemToInvoice('<?= $itemID ?>' ,<?= $stock ?>);" class="<?= $class ?>" data-item-id="<?= $itemID ?>"
    ...
    $subGroupEscaped = mysqli_real_escape_string($conn, $subGroup);
    $sql = "SELECT * FROM `itemscard` WHERE `subGroup` = '$subGroupEscaped' ORDER BY `stock` DESC";
